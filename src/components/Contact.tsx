    ...
    <section id="contact" className="relative py-28 md:py-36 bg-[color:var(--color-bg-2)]/40">
      <div className="container-x">
        ...
        <div className="mt-14 grid gap-6 lg:grid-cols-[1.3fr_1fr]">
          <Reveal>
            <form onSubmit={submit} className="glass-card rounded-3xl p-6 md:p-10">
              <div className="grid gap-4 md:grid-cols-2">
                <input required placeholder={t("contact.name")} className={inputCls} />
                <input required type="email" placeholder={t("contact.email")} className={inputCls} />
                <input placeholder={t("contact.phone")} className={inputCls} />
                <input placeholder={t("contact.profession")} className={inputCls} />
                <select required defaultValue="" className={inputCls + " md:col-span-2"}>
                  <option value="" disabled>{t("contact.course")}</option>
                  <option value="A1">A1</option>
                  <option value="A2">A2</option>
                  <option value="B1">B1</option>
                </select>
                <textarea rows={5} placeholder={t("contact.message")} className={inputCls + " md:col-span-2 resize-none"} />
              </div>
              <button
                disabled={sending}
          ...
          <div className="flex flex-col gap-5">
            <Reveal direction="right">
              <a href="https://wa.me/919876543210" target="_blank" rel="noreferrer" className="glass-card group flex items-center gap-5 rounded-3xl p-6 transition-all hover:border-[color:var(--color-gold)]/40">
            ...
            <Reveal direction="right" delay={0.1}>
              <a href="https://instagram.com" target="_blank" rel="noreferrer" className="glass-card group flex items-center gap-5 rounded-3xl p-6 transition-all hover:border-[color:var(--color-gold)]/40">
                <div className="grid h-14 w-14 place-items-center rounded-2xl bg-[color:var(--color-gold)]/10 text-[color:var(--color-gold)]">
                  <Instagram size={22} />
                </div>
                <div>
                  <div className="font-display text-xl text-white">Instagram</div>
                  <div className="text-sm text-[color:var(--color-paragraph)]">{t("contact.instagram")}</div>
                  <div className="text-xs text-white/40 mt-1">{t("contact.instagramSub")}</div>
                </div>
              </a>
            </Reveal>
            <Reveal direction="right" delay={0.2}>
              <a href="mailto:hello@keegermanacademy.com" className="glass-card group flex items-center gap-5 rounded-3xl p-6 transition-all hover:border-[color:var(--color-gold)]/40">
                <div className="grid h-14 w-14 place-items-center rounded-2xl bg-[color:var(--color-gold)]/10 text-[color:var(--color-gold)]">
                  <Mail size={22} />
                </div>
                <div>
                  <div className="font-display text-xl text-white">Email</div>
                  <div className="text-sm text-[color:var(--color-paragraph)]">{t("contact.email_card")}</div>
